...
function Actions({
  ...
  return (
    <Row style={{ justifyContent: "flex-end", gap: 8 }}>
      ...
      <RefButton label={submitLabel} kind="primary" onPress={onSubmit} />
    </Row>
  );
}
            ...
            return (
              <Pressable
                ...
                style={{
                  ...
                  borderWidth: 1,
                  borderColor: on ? REF.cyan : c.border,
                  backgroundColor: on ? REF.cyan : c.surface,
              ...
              >
                <Txt size={12} bold color={on ? REF.actionInk : c.text}>
                  {d}
                </Txt>
              </Pressable>
            );
...
export function hostelRecord(
  ...
  const keepStaffing = previous && !previous.setup && !h.wardens.length;
...
export function LeaveForm({
  ...
  const errors = useMemo(() => {
    ...
    else if (!DATE.test(l.start_date)) e.start_date = "Use YYYY-MM-DD";
    if (!l.end_date) e.end_date = "End date is required";
    else if (!DATE.test(l.end_date)) e.end_date = "Use YYYY-MM-DD";
