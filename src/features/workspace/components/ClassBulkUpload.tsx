...
import { downloadTemplate, pickCsv } from "../../../shared/files/classCsv";
...
export function ClassBulkUpload({
  ...
  const result = useMemo(
    () => checkUpload(table, existingNames, kind),
    [table, existingNames, kind],
  );
  ...
  return (
    <View style={{ gap: 14 }}>
      <Txt size={12} color={c.muted}>
        Download the template, fill in your {NOUN[kind].many}, then choose the
        CSV to review it before saving.
      </Txt>
      ...
      <Txt size={12} color={c.muted}>
        Required columns: {kind}_name, faculty_email, Tag. Duplicate rows are
          ...
          >
            <Row style={{ justifyContent: "space-between" }}>
              <Txt size={13} bold lines={1} style={{ flex: 1 }}>
                {row.data.class_name || `Row ${row.key + 2}`}
              </Txt>
              <Badge
                label={row.status}
                tone={
      ...
      <Row style={{ justifyContent: "flex-end", flexWrap: "wrap" }}>
        ...
        <Button
          variant="primary"
          label={`Save ${valid.length} ${NOUN[kind].many}`}
