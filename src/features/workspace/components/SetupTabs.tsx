...
import { Field, Row, Txt, Badge } from "../../../shared/ui/Primitives";
import { Icon } from "../../../shared/ui/Icon";
import { Select } from "../../../shared/ui/Select";
import { Dialog } from "../../../shared/ui/Dialog";
import { REF, RefButton } from "./referenceUi";
import { CAMERA_BRANDS } from "./CameraSetup";
...
let settings: SurveillanceSettings = {
  modules: { gate: false, analytics: false, video: false },
  notify: { web: channel(), email: channel(), sms: channel() },
  general: { fields: [], keepSurveillance: "7", keepAttendance: "7" },
  criteria: {},
  saved: false,
};
...
export const useSettings = () =>
  useSyncExternalStore(subscribe, () => settings);
const saveSettings = (next: Partial<SurveillanceSettings>) => {
  settings = { ...settings, ...next, saved: true };
          ...
          return (
            <Pressable
              ...
              style={{
                ...
                borderRadius: 99,
                borderWidth: 1,
                borderColor: on ? REF.cyan : c.border,
...
function Actions({
  ...
  return (
    <Row style={{ justifyContent: "flex-end", gap: 8 }}>
      {onCancel && <RefButton label="Cancel" onPress={onCancel} />}
      <RefButton label={label} kind="primary" onPress={onSave} />
    </Row>
  );
}
    ...
    <View style={{ gap: 14 }}>
      ...
      >
        ...
        >
          <Panel
            title="Module setup"
            subtitle="Gate attendance can be combined with one surveillance module."
          >
            <Toggle
            ...
            <Toggle
              ...
              onChange={(analytics) => setModules((m) => ({ ...m, analytics }))}
            />
            <Toggle
              label="Surveillance Video and Analytics"
              hint="Adds live video and recordings to the surveillance dashboard."
              value={modules.video}
              onChange={(video) => setModules((m) => ({ ...m, video }))}
            />
            <ErrorLine>{moduleError}</ErrorLine>
            <Actions label="Save" onSave={saveModules} />
          </Panel>
        </View>
        <View
          style={{
        ...
        >
          <Panel
            title="Notification"
            subtitle="Channels used when a recognised person matches a notification rule."
            action={
              <RefButton
      ...
      <Panel
        title="General setup"
        subtitle="Custom user fields and how long data is kept."
      >
        {[
          ["fields", "Other user fields", `${s.general.fields.length} fields`],
          [
            "surveillance",
            "Number of days to keep surveillance data",
            `${s.general.keepSurveillance} days`,
          ],
          [
            "attendance",
            "Number of days to keep attendance data",
      ...
      {advanced && (
        <Dialog
          title="Advance notification setting"
        ...
        {(["web", "email", "sms"] as const).map((k, i) => (
          <Pressable
            ...
            style={{
              ...
              alignItems: "center",
              borderWidth: 1,
              borderColor: open === k ? REF.cyan : c.border,
              backgroundColor: open === k ? REF.cyan : c.surface,
          ...
          >
            <Txt size={12} bold color={open === k ? REF.actionInk : c.text}>
  ...
  return (
    ...
    >
      {CRITERIA.map(([group, hint, fields]) => (
        <View
          ...
          style={{
            ...
            borderColor: c.border,
            paddingTop: group === "Brightness" ? 0 : 12,
          }}
        >
          <Txt size={13} bold>
            {group}
      ...
      <Row style={{ justifyContent: "flex-end", gap: 8 }}>
        ...
        />
        <RefButton label="Save" kind="primary" onPress={save} />
      </Row>
    </Panel>
  );
}
...
export function setupCameraRecord(
  ...
  const cells: Record<string, string> = {
    ...
    status:
      previous && !previous.setup ? text(previous.cells.status) : "Not active",
    state:
      previous && !previous.setup
        ? text(previous.cells.state)
        : "Not connected",
  };
  return {
    ...(previous ?? {}),
...
export function DashboardView({
  ...
  const here = rows.filter((d) => atCamera(d, cam));
    ...
    <View style={{ gap: 14 }}>
        ...
        {cameras.map((x, i) => (
          <Pressable
            ...
            style={{
              ...
              borderRadius: 99,
              borderWidth: 1,
              borderColor: i === active ? REF.cyan : c.border,
              backgroundColor: i === active ? REF.cyan : c.surface,
            }}
          >
            <Txt size={12} bold color={i === active ? REF.actionInk : c.text}>
            ...
            {s.modules.video ? (
              ...
              >
                <Txt size={12} color={c.muted} style={{ textAlign: "center" }}>
                  Live video needs the camera streaming service, which is not
      ...
      <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 10 }}>
        {[
          ...
          ["Identified", count("Identified"), REF.green],
          ["Unidentified", count("Unidentified"), REF.amber],
          ...
          >
            ...
            <Txt size={24} bold color={color as string}>
              {String(value)}
            </Txt>
          </View>
        ))}
      ...
      <Panel
        title={tab === "analytics" ? "Analytics" : "Video"}
        action={
          <Row style={{ gap: 6 }}>
            <RefButton
              label="Analytics"
              kind={tab === "analytics" ? "primary" : "plain"}
              onPress={() => setTab("analytics")}
            />
            <RefButton
              label="Video"
              kind={tab === "video" ? "primary" : "plain"}
          ...
          <>
            ...
            {shown.length ? (
              shown.map((d) => (
                ...
                >
                  <Txt size={12} bold style={{ flexBasis: 110 }}>
                    {text(d.cells.uid)}
                  </Txt>
                  <Txt size={12} style={{ flexGrow: 1, flexBasis: 150 }}>
                    {text(d.cells.name)}
                  </Txt>
                  <Badge label={text(d.cells.type)} tone={d.state.tone} />
                  <Txt size={12} color={c.muted} style={{ flexBasis: 80 }}>
                    {text(d.cells.time)}
                  </Txt>
                </Row>
              ))
